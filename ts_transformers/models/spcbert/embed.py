...
if __name__ == "__main__":
    ...
    def test_DAIN_Layer(batch_size, seq_len, dim):
        print("Start to test DAIN Layer...")
        modes = ["avg", "adaptive_avg", "adaptive_scale", "full"]
        x = torch.arange(batch_size * seq_len * dim,
                         dtype=torch.float).reshape(batch_size, dim, seq_len)

        for mode in modes:
            print(f"    Test {mode}...")
            norm = DAIN_Layer(mode=mode, input_dim=dim)
            with torch.no_grad():
                y = norm(x)
                y_denormalize = norm.denormalize(y)
        ...
        x = x.to(device)
        for mode in modes:
            norm = DAIN_Layer(mode=mode, input_dim=dim).to(device)
            with torch.no_grad():
                y = norm(x)
                y_denormalize = norm.denormalize(y)
            err = torch.mean((x - y_denormalize)**2)
    ...
    test_DAIN_Layer(batch_size, seq_len, dim)
