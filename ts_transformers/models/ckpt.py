...
class EarlyStopping:
    ...
    def __call__(self, val_accuracy, model) -> None:
        score = val_accuracy
        ...
        if self.target == "accuracy":
            ...
            raise NotImplementedError(f"Not implemented {self.target} in EarlyStopping.")
    ...
    def save_checkpoint(self, val_accuracy, model):
        ...
        if self.verbose:
            if self.target == 'accuracy':
                ...
                display = "decreased"
            self.trace_func(
                f'Validation {self.target} {display} ({self.val_accuracy_min:.6f} --> {val_accuracy:.6f}).  Saving model ...\n'
            )
